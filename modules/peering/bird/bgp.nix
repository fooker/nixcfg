...
let
  neighbors = concatMapStringsSep "\n"
    (peer:
      ...
      in
      if peer.domains.${domain.name}.bgp.multiprotocol
      then
        ''
          protocol bgp ${domain.name}_bgp_${name} from ${domain.name}_bgp {
...
''
  ...
  template bgp ${domain.name}_bgp_base {
  ...
  template bgp ${domain.name}_bgp_4 from ${domain.name}_bgp_base {
    ipv4 {
      table ${domain.name}_egp_4;
  ...
  template bgp ${domain.name}_bgp_6 from ${domain.name}_bgp_base {
    ipv6 {
      table ${domain.name}_egp_6;
      preference ${toString domain.bgp.preference};
  ...
  template bgp ${domain.name}_bgp from ${domain.name}_bgp_base {
    ipv4 {
      table ${domain.name}_egp_4;
      preference ${toString domain.bgp.preference};
