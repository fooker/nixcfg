...
{
  # Gives an attrset of names to machines by recursively finding machine.nix
  # files in the ./machines directory.
  flake.lib.machines =
    let
      path = id: rec {
        # Relative path for the id
        relPath = "machines" + (concatMapStrings (s: "/${s}") id);
